...
app.post("/compile", function (req, res) {
    ...
    try {
        ...
        if (lang === "Python") {
            ...
            // Extract class name from the code (this assumes the first class defined is the public class)
            const classNameMatch = code.match(/class\s+(\w+)/);
            const className = classNameMatch ? classNameMatch[1] : "Main"; // Default to Main if not found

            // Modify envData to include the class name
            envData.fileName = className + ".java";
